...
public class RuntimeEntrypoints {
  ...
  //  Concurrency   //
  //----------------//
  // @param o: the object whose field is being accessed
  // @param read: the type of access. 1 for read, 0 for write
  
  @Entrypoint
  public static void threadCheck(Object o, int read) throws Exception
  {
    if(o != null)
    {
       ...
       switch(permission) {
         case 0: //Private
           if (curID != ownerID) {
             VM.sysWriteln(String.format("Intentional Concurrency: Invalid access on private object. Object type: %s. (Owner: %d, Accessor: %d)", 
                                  o.getClass(), ownerID, curID));
         ...
         case 1: //Frozen
           ...
           if (read == 0) {
             VM.sysWriteln(String.format("Intentional Concurrency: Invalid write to frozen object. Object type: %s. (Owner: %d, Accessor: %d)",
                                                o.getClass(), ownerID, curID));
             athrow(new Exception());
         }
           break;
         case 2: //Transfer
           //Read/Write -> Change Permission of object to private and thread to current thread
           if(curID != ownerID)
           {
             o.setOwningThread(curID);
             o.setPermission(Object.ConcurrencyPermission.PRIVATE);
           }
           break;
         case 3: //Loan?
         ...
         case 4: //SameAs(Leader)?
           ...
           break;
         //Nothing required for these cases
         case 5: //ThreadSafe
        ...
        //Can be used to throw an exception, if desired in the future
